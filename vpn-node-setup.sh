...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v4.8 (All-in-One)
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v4.8 (All-in-One)${NC}"
...
# Эти проверки находят и чинят 3 типичные проблемы которые приводят к тому,
...
#
#   3. Дубликаты в /etc/fstab
#      Проблема: повторные строки swapfile вызывают ошибки systemd-fstab-generator
#      Симптом: предупреждения при загрузке, иногда блокировка mount-юнитов
...
# Generated by vpn-node-setup-v4.6 — replaces broken cloud-init netplan
...
if [ -f /etc/fstab ]; then
    # Ищем дубликаты по комбинации первого поля (source) + второго (target)
    # Это правильный способ потому что у swap target = "none", и одна точка / может
    # иметь только один источник
    DUPLICATES=$(awk '
        !/^#/ && !/^[[:space:]]*$/ {
            key = $1 "|" $2
            if (seen[key]) {
    ...
    if [ -n "$DUPLICATES" ]; then
        print_warn "Найдены дубликаты в /etc/fstab:"
        for dup in $DUPLICATES; do
            echo -e "    ${YELLOW}⚠${NC} $dup"
        done

        # Бэкап
        cp /etc/fstab "$BACKUP_DIR/fstab.backup" 2>/dev/null

        # Удаляем дубликаты — оставляем первое вхождение каждой пары source+target
        awk '
            /^#/ || /^[[:space:]]*$/ { print; next }
            {
                key = $1 "|" $2
        ...
        # Проверяем что не сломали (в файле остались записи)
        if [ -s /tmp/fstab.fixed ] && grep -q "^[^#]" /tmp/fstab.fixed; then
            mv /tmp/fstab.fixed /etc/fstab
            print_ok "Дубликаты удалены (бэкап: $BACKUP_DIR/fstab.backup)"
        else
...
# --- Защита 8: systemd-networkd-wait-online нестабильность ---
# Сервис ждёт ВСЕ интерфейсы (включая несуществующие из старых конфигов).
# Если в системе несколько интерфейсов или были изменения — он может падать
# с timeout. Это блокирует network-online.target → Xray может стартовать
# до готовности сети.
#
# Решение: override который ждёт только default-интерфейс с timeout 30 сек.
WAIT_ONLINE_FIXED=0
if [ "$NETWORKD_ACTIVE" -eq 1 ] && [ -n "$DEFAULT_IFACE" ] && [ "$DEFAULT_IFACE" != "lo" ]; then
    print_status "Проверяем systemd-networkd-wait-online..."

    WAIT_ONLINE_STATE="unknown"
    if systemctl is-failed systemd-networkd-wait-online.service >/dev/null 2>&1; then
        WAIT_ONLINE_STATE="failed"
    elif systemctl is-active systemd-networkd-wait-online.service >/dev/null 2>&1; then
        WAIT_ONLINE_STATE="active"
    fi

    # Проверяем — есть ли уже наш override от админа
    OVERRIDE_DIR="/etc/systemd/system/systemd-networkd-wait-online.service.d"
    OVERRIDE_FILE="$OVERRIDE_DIR/vpn-node-override.conf"
    HAS_ADMIN_OVERRIDE=0

    if [ -d "$OVERRIDE_DIR" ]; then
        # Если есть other override-файлы (не наш) — не трогаем
        for f in "$OVERRIDE_DIR"/*.conf; do
            [ -f "$f" ] || continue
            [ "$f" != "$OVERRIDE_FILE" ] && HAS_ADMIN_OVERRIDE=1
        done
    fi

    if [ "$HAS_ADMIN_OVERRIDE" -eq 1 ]; then
        print_info "Найден admin-override для wait-online — не трогаем"
    elif [ "$WAIT_ONLINE_STATE" = "failed" ] || [ ${#NETPLAN_ISSUES[@]} -gt 0 ] || [ ! -f "$OVERRIDE_FILE" ]; then
        # Применяем фикс если:
        # - Сервис уже failed
        # - Были проблемы с netplan (после фикса нужно настроить wait-online на правильный iface)
        # - Нашего override ещё нет (превентивно)
        if [ "$WAIT_ONLINE_STATE" = "failed" ]; then
            print_warn "systemd-networkd-wait-online в failed-состоянии"
        elif [ ! -f "$OVERRIDE_FILE" ]; then
            print_status "Создаём override для надёжности (ждём только $DEFAULT_IFACE с timeout 30s)"
        fi

        mkdir -p "$OVERRIDE_DIR"
        cat > "$OVERRIDE_FILE" <<EOF
# Generated by vpn-node-setup-v4.8
# Wait only for default interface to be online — prevents hang on missing/old interfaces
[Service]
ExecStart=
ExecStart=/usr/lib/systemd/systemd-networkd-wait-online --interface=$DEFAULT_IFACE --timeout=30
EOF

        systemctl daemon-reload 2>/dev/null

        if [ "$WAIT_ONLINE_STATE" = "failed" ]; then
            systemctl reset-failed systemd-networkd-wait-online.service 2>/dev/null
            # Перезапускаем чтобы сразу применился override
            if systemctl restart systemd-networkd-wait-online.service 2>/dev/null; then
                print_ok "Override применён, сервис перезапущен"
                WAIT_ONLINE_FIXED=1
            else
                # Не падаем — после ребута заработает
                print_info "Сервис применит override при следующем boot"
                WAIT_ONLINE_FIXED=1
            fi
        else
            print_ok "Override создан (применится при следующем boot)"
            WAIT_ONLINE_FIXED=1
        fi
    else
        print_ok "systemd-networkd-wait-online работает корректно"
    fi
fi
...
[ "$WAIT_ONLINE_FIXED" -eq 1 ] && echo -e "    • wait-online: ${YELLOW}override применён${NC}" || echo -e "    • wait-online: ${GREEN}OK${NC}"
echo ""
