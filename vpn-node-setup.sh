...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v5.0 (UNIVERSAL: оптимизация + диагностика)
...
v5_do_check() {
    ...
    if [ "$upstream_ver" = "$SCRIPT_VERSION" ]; then
        ...
        if [ "$newer" = "$upstream_ver" ]; then
            ...
            echo ""
            # При запуске через `bash <(curl ...)`, $0 будет /dev/fd/N — не путай юзера.
            local self_path="$0"
            if [[ ! -f "$self_path" ]] || [[ "$self_path" == /dev/fd/* ]] || [[ "$self_path" == bash ]]; then
                self_path="vpn-node-setup.sh"
            fi
...
for arg in "$@"; do
    case "$arg" in
        ...
        --upgrade)
            ...
            echo "  Если что-то пойдёт не так: sudo bash $0 --rollback"
            echo ""
            sleep 2
...
Examples:
  sudo $0                  # TTY: TUI меню. non-TTY: оптимизация.
  sudo $0 --optimize       # Прямая оптимизация без меню (CI/ansible).
  sudo $0 --diagnose       # Прямая диагностика без меню.
  sudo $0 --dry-run        # Посмотреть что будет
  $0 --check               # Проверить новую версию
  sudo $0 --upgrade        # Безопасный self-upgrade
...
diag_run() {
    ...
    chmod +x "$tmp"
    print_ok "node-diagnostic прошёл sanity-check, запускаю..."
    echo ""
    bash "$tmp"
    local rc=$?
    echo ""
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v5.0 (Universal: Optimize + Diagnose)${NC}"
...
# Conntrack tuning for VPN node (gaming-friendly) — v5.0 tier-aware
# Profile: $CONNTRACK_TIER (RAM ${CONNTRACK_TOTAL_MEM_MB}MB)
# UDP timeouts здесь намеренно отсутствуют — управляются shieldnode (180/300).
...
EOF
print_ok "Conntrack настроен и сохранён (max=$CONNTRACK_MAX)"
...
echo -e "  ${CYAN}sudo bash $0 --check${NC}                        # Проверить новую версию на github"
echo -e "  ${CYAN}sudo bash $0 --upgrade${NC}                      # Безопасный upgrade (snapshot + rollback support)"
echo -e "  ${CYAN}sudo bash $0 --rollback${NC}                     # Откатиться к предыдущей версии"
echo ""
