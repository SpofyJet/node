...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "  --dry-run  Не устанавливать ядро через apt; только показать"
            echo "             что было бы сделано. Sysctl/лимиты применяются"
            echo "             в обычном режиме (их можно откатить руками)."
