...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v5.0.7 (Universal: Optimize + Diagnose)${NC}"
...
KNOWN_LTS_BRANCHES=("5.4" "5.10" "5.15" "6.1" "6.6" "6.12" "6.18")
...
# === TCP Window Scale (v5.1.0 — moved here from TIER 3/4) ===
# Больше окна под данные, меньше под метаданные → +5-15% effective
# rmem для forwarding нагрузки. Эмпирически проверено на проде —
# выгодно на всех tier, не только на больших.
...
# v4.13 CRIT-2: rmem_max 2MB → 4MB. UDP-based BBR (QUIC/Hysteria/etc) требует
# rmem_max >= 4MB чтобы выйти на потолок throughput. 2MB давало 12-25% от
# потолка (rwnd cap'нут на 2MB → BBR не может open larger window).
...
# v4.13 CRIT-2: rmem_max 8MB → 16MB. Та же проблема что в TIER 1: high-bandwidth
# UDP (QUIC, Hysteria) requires >= 16MB на 2GB ноде для full BBR throughput.
# 8MB давало ~50% потолка.
...
# (shieldnode+CrowdSec+Xray+QUIC+bouncer baseline ~1.5GB + bursts 200-300MB).
