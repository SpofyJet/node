...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v4.13 (All-in-One, MINIMAL-RISK EDITION)
#  Ядро XanMod LTS + BBRv3 + Полная оптимизация системы
...
# v4.13: версия + repo URL для self-upgrade
SCRIPT_VERSION="4.13"
...
for arg in "$@"; do
    case "$arg" in
        ...
        --check)
            ...
            CHECK_TMP=$(mktemp -t vpn-node-check.XXXXXX) || { echo "FATAL: mktemp failed"; exit 1; }
            ...
            if [ "$UPSTREAM_VER" = "$SCRIPT_VERSION" ]; then
                ...
                if [ "$NEWER" = "$UPSTREAM_VER" ]; then
                    ...
                    echo ""
                    echo "Запустить upgrade: sudo bash $0 --upgrade"
                    echo "Diff с текущей:    sudo bash $0 --diff"
                else
        ...
        --upgrade)
            ...
            trap - EXIT
            # Передаём оставшиеся аргументы (кроме --upgrade) новой версии
            exec bash "$SCRIPT_INSTALLED_PATH"
            ;;
        --rollback)
            if [[ $EUID -ne 0 ]]; then
...
Без аргументов:    Запустить установку/обновление настроек ноды.
...
Examples:
  sudo $0                  # Обычная установка/обновление
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v4.13 (Minimal-Risk Edition)${NC}"
