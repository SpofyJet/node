...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v5.0.5 (UNIVERSAL: оптимизация + диагностика)
#  Ядро XanMod LTS + BBRv3 + Полная оптимизация системы + MSS clamp + Diagnostics
#  Поддерживает: Debian 12/13, Ubuntu 22.04/24.04
#
#  v5.0.5 changelog (HEADLINE FIX: YouTube freeze / video stalls):
#
#  - Remove: net.ipv4.tcp_notsent_lowat = 131072 (возврат к kernel default).
#    КОНТЕКСТ: на нодах со скриптом юзеры жаловались на короткие фризы видео
#    (YouTube, Twitch, и др.) — на нодах без скрипта проблемы нет.
#    КОРЕНЬ: notsent_lowat=131072 ограничивает unsent данные в TCP socket
#    до 128 KB. На веб-сервере (Cloudflare ставит то же значение) это работает
#    корректно — там user-space контролирует write rate под HTTP/2 priorities.
#    У нас Xray читает большие TLS chunks из CDN-socket и пишет в client-socket.
#    При unsent ≥ 128 KB write() блокируется → Xray ОСТАНАВЛИВАЕТ чтение от
#    CDN → CDN снижает rate → клиентский видео-буфер drain → freeze.
#    FIX: возврат к kernel default (UINT_MAX, ~4 GB ≈ unlimited).
#    Autotuning + BBR pacing сами управляют sending rate, watermark на
#    app-level не нужен для forwarding workload.
#    Источники: kernel.org docs, Cloudflare blog (Oct 2025), Eric Dumazet.
#
#  - Change: gro_flush_timeout 50µs → 0, napi_defer_hard_irqs 1 → 0
#    (возврат к classic NAPI без deferred IRQs).
#    КОНТЕКСТ: defer добавлял 50µs jitter на TX path. Kernel docs прямо
#    говорят: "будет inducd latency when the system is not fully loaded".
#    На VPN-relay с 50-200 активных юзеров система обычно "not fully loaded".
#    50µs jitter сам по себе не вызвал бы freeze, но в комбинации с
#    notsent_lowat усиливал ощущение micro-stall.
#    Экономия CPU 15-25% softirq заметна только при 1000+ юзеров на ноду —
#    для типичной нагрузки не оправдывает jitter.
#    Если нода реально перегружена — можно вернуть значения вручную (см.
#    комментарии в коде).
#
#  ВАЖНО: после применения тестируй YouTube/streaming на 2-3 нодах перед
#         массовым роллаутом. Throughput остаётся прежним (BBR + tier-aware
#         buffers + qdisc fq не трогаются), но устраняется periodic stall.
#         Если фризы остались — проблема не в скрипте, искать в Xray config,
#         CrowdSec false-positive bans на CDN edges, или MTU mismatch.
#
#  v5.0.4 changelog (ARCH SIMPLIFICATION + CRITICAL/HIGH bug fixes):
#
#  АРХИТЕКТУРА (убрать пересечения с shieldnode v3.20.5):
#  - Remove: 6 sysctl-ключей которые дублировали shieldnode (rp_filter,
#    tcp_syncookies, accept_redirects, send_redirects, icmp_echo_ignore_broadcasts,
#    tcp_rfc1337). Раньше vpn-node-setup писал в 99-vpn-node-tuning.conf и
#    лексикографически перетирал 90-shieldnode.conf при reboot. Теперь
#    shieldnode единственный владелец этих ключей.
#  - Remove: iptables fallback для MSS clamp в ШАГ 7.8. Раньше при отсутствии
#    nft использовался iptables-mangle + iptables-save в /etc/iptables/rules.v4 —
#    это дампило ВСЕ правила (включая UFW и shieldnode) → конфликты при reboot.
#    Безопаснее fail clean чем тихо ломать firewall. Если nft нет — install
#    его перед запуском (`apt install nftables`).
#  - Change: atomic nft transaction для MSS clamp (live + systemd unit + conf).
#    Раньше: nft delete + add создавали окно ~10-100ms без правил → SYN-пакеты
#    проходили без MSS clamp. Теперь: 'add table {}' → 'delete table' →
#    'add table {...}' в одной nft -f транзакции (атомарно для kernel).
#  - Change: ExecStop= в mss-clamp.service с префиксом `-` (игнорирует exit
#    code если внешний процесс уже flush'нул таблицу — unit не уйдёт в failed).
#
#  CRITICAL bug fixes:
#  - Fix #35: assign DEFAULT_IFACE в ШАГ 1.5. Раньше переменная использовалась
#    в NIC driver compatibility check (ШАГ 4 после kernel install) но НИКОГДА
#    не присваивалась → проверка молча скипалась. На vendor-kernel'ах (Alibaba/
#    Yandex Cloud) это могло привести к unbootable серверу после reboot если
#    driver не вкомпилен в новый XanMod LTS.
#  - Fix #36: GPG ключ XanMod скачивается ДО удаления старых ключей + fallback
#    на keyserver.ubuntu.com если dl.xanmod.org недоступен (CF 403, network).
#    Раньше: rm old keys → wget → install new. Если wget падал, система
#    оставалась без keyring + xanmod-release.list уже добавлен → broken state.
#
#  HIGH bug fixes:
#  - Fix #13: $(date) → $(date -u +%Y-%m-%d) в sysctl heredoc. Раньше timestamp
#    с секундной точностью делал --diff всегда non-empty.
#  - Fix #15: guard против custom qdisc (htb/cake/cbq/hfsc/codel/pie/netem/tbf)
#    в ШАГ 7.5. Раньше `tc qdisc replace` молча wipe'ил admin'ский config.
#  - Fix #27: systemctl stop --no-block для apt services/timers. Раньше мог
#    ждать graceful stop до 90 сек на каждый unit — install висел минуты.
#  - Fix #28: cleanup xanmod-release.list + keyring при apt-get update fail.
#    Раньше: следующий запуск тоже падал на apt-get update из-за broken repo.
#  - Fix #37: wget --timeout=15 --tries=2. Раньше мог висеть 15 минут на
#    CF 403 от dl.xanmod.org.
#  - Fix #42: glob '*lts*xanmod*' вместо '*xanmod*' для post-install initrd
#    validation. Раньше: если параллельно стоял MAIN xanmod (v6.20+), sort -V
#    выбирал MAIN вместо нашего LTS, и проверка validate'ила не тот файл.
#  - Fix #52: atomic --upgrade transaction. Bash в subprocess вместо exec,
#    promotion installed.sh только при success. Раньше: если новая версия
#    падала на dpkg integrity check — installed.sh уже указывал на неё.
#    Цена: родительский bash остаётся в памяти на 5-15 мин (вместо exec).
#  - Fix #57: regex поддерживает patch-level (5.0.3, а не только 5.0) в
#    --check / --upgrade / --rollback. Раньше hotfix-апгрейды не работали.
#
#  ВАЖНО: после применения убедись что shieldnode v3.20.5+ установлен —
#         иначе security sysctl-ключи (rp_filter, syncookies, etc) останутся
#         на kernel defaults. Если shieldnode НЕТ — добавь ключи вручную
#         (см. комментарий "Security Hardening" в sysctl heredoc).
#
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v5.0.5 (Universal: Optimize + Diagnose)${NC}"
