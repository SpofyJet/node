...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v4.4 (All-in-One)
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v4.4 (All-in-One)${NC}"
...
# Верифицируем fingerprint ключа (защита от MITM/подмены)
XANMOD_FINGERPRINT="BBFD DB86 3E27 23E3 3371  99DC 4F78 1D3D 0E28 E4DB"
ACTUAL_FP=$(gpg --with-fingerprint --with-colons "$XANMOD_KEY_TMP" 2>/dev/null | awk -F: '/^fpr:/{print $10; exit}' | sed 's/.\{4\}/& /g' | xargs)
# Нормализуем для сравнения (убираем пробелы)
EXPECTED_CLEAN=$(echo "$XANMOD_FINGERPRINT" | tr -d ' ')
ACTUAL_CLEAN=$(echo "$ACTUAL_FP" | tr -d ' ')

if [ -z "$ACTUAL_CLEAN" ]; then
    print_error "Не удалось извлечь fingerprint GPG ключа!"
    rm -f "$XANMOD_KEY_TMP"
    exit 1
fi

if [ "$ACTUAL_CLEAN" != "$EXPECTED_CLEAN" ]; then
    print_error "SECURITY: GPG fingerprint не совпадает!"
    echo -e "    ${RED}Ожидался:  $XANMOD_FINGERPRINT${NC}"
    echo -e "    ${RED}Получен:   $ACTUAL_FP${NC}"
    print_error "Установка прервана. Возможна атака MITM или смена ключа."
    rm -f "$XANMOD_KEY_TMP"
    exit 1
fi

print_ok "Fingerprint GPG ключа верифицирован: OK"
