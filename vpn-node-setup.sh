...
#  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
#                                                         
#  XRAY/REMNAWAVE NODE BUILDER v4.9 (All-in-One)
...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ "
echo -e "${NC}"
echo -e "${BOLD}  XRAY/REMNAWAVE NODE BUILDER v4.9 (All-in-One)${NC}"
...
# ==============================================================================
# ШАГ 1.5: PRE-FLIGHT CHECKS (защита от типовых поломок после ребута)
# ==============================================================================

print_header "ШАГ 1.5: PRE-FLIGHT ПРОВЕРКИ СЕТИ И FSTAB"

# Эти проверки находят и чинят типичные проблемы которые приводят к тому,
# что сервер не загружается после ребута:
#
#   1. Netplan ссылается на несуществующий интерфейс (enp1s0 вместо ens3 etc)
#      Проблема: cloud-init иногда генерирует кривой конфиг под другой провайдер
#      Симптом: networking.service failed, нужен hard reset через console
#
#   2. networking.service (ifupdown) активен одновременно с systemd-networkd
#      Проблема: гонка между двумя сервисами — кто первый поднимет интерфейс
#      Симптом: нестабильная загрузка, иногда сеть не поднимается
#
#   3. Дубликаты в /etc/fstab (особенно swap-записи /swapfile + /swapfile2)
#      Проблема: повторные swap-строки вызывают ошибки systemd-fstab-generator
#      Симптом: Failed to create unit file '/run/systemd/generator/swapfile.swap'
#
#   8. systemd-networkd ждёт IPv6 RA при dhcp6: false (KVM с virtio_net)
#      Проблема: networkd держит интерфейс в "configuring" 30с пока NDISC
#      timeout не отработает → systemd-networkd-wait-online failed
#      Симптом: "Timeout occurred while waiting for network connectivity"
#      Фикс: accept-ra: false + link-local: [ ipv4 ] в netplan
#
#   9. /etc/netplan/00-installer-config.yaml от subiquity
#      Проблема: при автообновлении может содержать ссылки на enp1s0
#      когда реальный интерфейс ens3 (как в v4.0)
#      Решение: нейтрализуем (бэкап + пустой каркас)
#
#  10. /etc/network/interfaces непустой при активном systemd-networkd
#      Проблема: ifupdown пытается управлять ens3 параллельно
#      Симптом: ifup@ens3.service failed

# --- Защита 0: dpkg integrity ---
# КРИТИЧНО: если dpkg в битом состоянии — установка ядра упадёт посреди процесса
...
if [ -d "$NETPLAN_DIR" ] && ls "$NETPLAN_DIR"/*.yaml >/dev/null 2>&1; then
    ...
    if [ -z "$DEFAULT_IFACE" ]; then
        ...
        print_info "Default интерфейс: $DEFAULT_IFACE"

        # Проверяем каждый netplan yaml на упоминание интерфейсов
        for yaml_file in "$NETPLAN_DIR"/*.yaml; do
            [ -f "$yaml_file" ] || continue

            # Извлекаем имена интерфейсов из ethernets: блока
            # Корректно отслеживаем индентацию чтобы не захватить bridges/vlans/etc
            DEFINED_IFACES=$(awk '
                ...
                in_eth {
                    ...
                    cur_indent = RLENGTH
                    if (NF == 0) next
                    # Вышли из блока ethernets (меньшая или равная индентация)
                    if (cur_indent <= eth_indent) {
                        in_eth = 0
                        if ($0 ~ /^([[:space:]]*)ethernets:[[:space:]]*$/) {
                            eth_indent = cur_indent
                            in_eth = 1
                            parent_indent = 0
                        }
                        next
                    }
                    # Прямой потомок (имя интерфейса)
                    if ($0 ~ /^[[:space:]]+[a-zA-Z][a-zA-Z0-9_-]+:[[:space:]]*$/) {
                        if (parent_indent == 0) parent_indent = cur_indent
            ...
            if [ -n "$DEFINED_IFACES" ]; then
                for iface in $DEFINED_IFACES; do
                    if ! echo " $REAL_IFACES " | grep -q " $iface "; then
                        NETPLAN_ISSUES+=("$yaml_file: ссылается на $iface (не существует)")
                    fi
                done
            fi
        done

        if [ ${#NETPLAN_ISSUES[@]} -gt 0 ]; then
            print_warn "Найдены проблемы в netplan:"
            for issue in "${NETPLAN_ISSUES[@]}"; do
                echo -e "    ${YELLOW}⚠${NC} $issue"
            done

            # Создаём бэкап и пересоздаём конфиг
            mkdir -p "$BACKUP_DIR/netplan-broken" 2>/dev/null
            cp "$NETPLAN_DIR"/*.yaml "$BACKUP_DIR/netplan-broken/" 2>/dev/null

            # Удаляем именно cloud-init файл, если он битый
            if [ -f "$NETPLAN_DIR/50-cloud-init.yaml" ] && \
               grep -q "to it will not persist" "$NETPLAN_DIR/50-cloud-init.yaml" 2>/dev/null; then
                BAD_IN_CLOUDINIT=0
                for issue in "${NETPLAN_ISSUES[@]}"; do
                    [[ "$issue" == *"50-cloud-init.yaml"* ]] && BAD_IN_CLOUDINIT=1
                done
                if [ "$BAD_IN_CLOUDINIT" -eq 1 ]; then
                    print_status "Удаляем кривой /etc/netplan/50-cloud-init.yaml"
                    rm -f "$NETPLAN_DIR/50-cloud-init.yaml"
                    NETPLAN_FIXED=1
                fi
            fi

            # Создаём правильный конфиг для default интерфейса
            if [ "$NETPLAN_FIXED" -eq 1 ] || [ ! -f "$NETPLAN_DIR/00-installer-config.yaml" ]; then
                print_status "Создаём корректный netplan для $DEFAULT_IFACE"
                cat > "$NETPLAN_DIR/01-vpn-node-network.yaml" <<EOF
# Generated by vpn-node-setup-v4.9 — replaces broken cloud-init netplan
# Original (broken) configs backed up to $BACKUP_DIR/netplan-broken/
#
# accept-ra: false + link-local: [ ipv4 ] — критично для KVM с virtio_net.
# Без этого systemd-networkd ждёт IPv6 RA до timeout (30с) даже при
# dhcp6: false, и wait-online падает с "Timeout occurred".
# БЕЗ optional: true — на единственном интерфейсе он сам ломает wait-online
# (Launchpad #2060311, #2060689).
network:
  version: 2
  renderer: networkd
  ethernets:
    $DEFAULT_IFACE:
      ...
      link-local: [ ipv4 ]
EOF
                chmod 600 "$NETPLAN_DIR/01-vpn-node-network.yaml"

                # Валидируем netplan
                if netplan generate 2>&1 | tee /tmp/netplan-gen.log | grep -qiE "error|fail"; then
                    print_error "netplan generate выдал ошибки!"
                    print_info "Восстанавливаем оригинальные конфиги..."
                    rm -f "$NETPLAN_DIR/01-vpn-node-network.yaml"
                    cp "$BACKUP_DIR/netplan-broken/"*.yaml "$NETPLAN_DIR/" 2>/dev/null
                    NETPLAN_FIXED=0
                else
                    print_ok "Netplan починен (бэкап: $BACKUP_DIR/netplan-broken/)"
                fi
            fi
        else
            print_ok "Netplan корректен — все интерфейсы существуют"
        fi
    fi
else
    print_info "Netplan не используется (нет /etc/netplan/*.yaml) — пропускаем"
fi

# --- Защита 2: networking.service vs systemd-networkd конфликт ---
print_status "Проверяем конфликт networking.service vs systemd-networkd..."

NETWORKD_ACTIVE=0
NETWORKING_ACTIVE=0
systemctl is-active systemd-networkd >/dev/null 2>&1 && NETWORKD_ACTIVE=1
systemctl is-enabled networking.service >/dev/null 2>&1 && NETWORKING_ACTIVE=1

if [ "$NETWORKD_ACTIVE" -eq 1 ] && [ "$NETWORKING_ACTIVE" -eq 1 ]; then
    # Проверяем — может networking.service уже падает
    NETWORKING_FAILED=0
    if systemctl is-failed networking.service >/dev/null 2>&1; then
        NETWORKING_FAILED=1
    fi

    if [ "$NETWORKING_FAILED" -eq 1 ] || [ ${#NETPLAN_ISSUES[@]} -gt 0 ]; then
        print_warn "networking.service конфликтует с systemd-networkd"
        print_info "(networking.service failed=$NETWORKING_FAILED, netplan_issues=${#NETPLAN_ISSUES[@]})"
        print_status "Отключаем networking.service (старый ifupdown)"

        systemctl disable networking.service 2>/dev/null
        systemctl mask networking.service 2>/dev/null
        systemctl reset-failed networking.service 2>/dev/null
        # Также чистим failed ifup@ юниты
        for unit in $(systemctl list-units --failed --no-legend 2>/dev/null | awk '/ifup@/{print $2}'); do
            systemctl reset-failed "$unit" 2>/dev/null
        done
        print_ok "networking.service отключён, systemd-networkd остаётся главным"
    else
        print_ok "networking.service работает корректно — не трогаем"
    fi
elif [ "$NETWORKD_ACTIVE" -eq 1 ]; then
    print_ok "Используется systemd-networkd (конфликтов нет)"
elif [ "$NETWORKING_ACTIVE" -eq 1 ]; then
    print_ok "Используется networking.service (конфликтов нет)"
else
    print_info "Сетевой менеджер не определён (возможно NetworkManager)"
fi

# --- Защита 3: Дубликаты в /etc/fstab ---
print_status "Проверяем /etc/fstab на дубликаты..."

DUPLICATES=""
FSTAB_FIXED=0
if [ -f /etc/fstab ]; then
    # Логика v4.9:
    # 1. Все swap-записи (type=swap) считаем потенциальными дубликатами
    #    независимо от source/target — в системе может быть только один
    #    осмысленный swap-файл, остальные ломают systemd-fstab-generator.
    #    Старая логика v4.8 по source+target пропускала пары вроде
    #    "/swapfile swap swap" + "/swapfile2 none swap".
    # 2. Не-swap дубликаты ловим как раньше — по source+target.
    SWAP_COUNT=$(awk '!/^[[:space:]]*#/ && NF>=3 && $3=="swap"' /etc/fstab | wc -l)
    NONSWAP_DUPS=$(awk '
        !/^[[:space:]]*#/ && !/^[[:space:]]*$/ && NF>=3 && $3!="swap" {
            key = $1 "|" $2
            if (seen[key]) {
                print $1
            }
    ...
    [ "$SWAP_COUNT" -gt 1 ] && DUPLICATES="$DUPLICATES swap×$SWAP_COUNT"
    [ -n "$NONSWAP_DUPS" ] && DUPLICATES="$DUPLICATES $NONSWAP_DUPS"

    if [ -n "$DUPLICATES" ]; then
        print_warn "Найдены проблемы в /etc/fstab:"
        [ "$SWAP_COUNT" -gt 1 ] && echo -e "    ${YELLOW}⚠${NC} $SWAP_COUNT swap-записей (оставим первую)"
        for dup in $NONSWAP_DUPS; do
            echo -e "    ${YELLOW}⚠${NC} duplicate: $dup"
        done

        # Бэкап
        cp /etc/fstab "$BACKUP_DIR/fstab.backup" 2>/dev/null

        # Дедуп: первая swap остаётся, остальные комментируем;
        # не-swap дубликаты дедуплицируем по source+target как раньше.
        awk '
            BEGIN { swap_kept = 0 }
            ...
            $3 == "swap" {
                if (swap_kept == 0) {
                    ...
                    print "# vpn-node-setup v4.9: duplicate swap disabled -> " $0
                }
                next
            }
            {
        ...
        ' /etc/fstab > /tmp/fstab.fixed

        # Sanity check: должны остаться не-комментированные строки
        if [ -s /tmp/fstab.fixed ] && grep -q "^[^#]" /tmp/fstab.fixed; then
            mv /tmp/fstab.fixed /etc/fstab
            chmod 644 /etc/fstab
            FSTAB_FIXED=1
            print_ok "Fstab почищен (бэкап: $BACKUP_DIR/fstab.backup)"
            # Перегенерация systemd-юнитов из fstab без apply/swapon
            systemctl daemon-reload 2>/dev/null || true
        else
            rm -f /tmp/fstab.fixed
            print_warn "Не удалось безопасно очистить fstab — пропускаем"
        fi
    else
        print_ok "/etc/fstab без дубликатов"
    fi
fi

# --- Защита 4: Zombie cloud-init (uninstalled but configs remain) ---
print_status "Проверяем состояние cloud-init..."

CLOUDINIT_STATE=$(dpkg -l cloud-init 2>/dev/null | awk '/^.[ni]/{print substr($1,1,2); exit}')
# Сохраняем оригинальное состояние для финального отчёта
CLOUDINIT_ORIG_STATE="$CLOUDINIT_STATE"
case "$CLOUDINIT_STATE" in
    "ii")
        print_ok "cloud-init установлен и активен"
        ;;
    "un"|"rc")
        # un = uninstalled, configs remain
        # rc = removed, configs remain
        print_warn "cloud-init в zombie-состоянии: $CLOUDINIT_STATE (uninstalled, но конфиги остались)"
        if [ -d /etc/cloud ] || [ -d /var/lib/cloud ]; then
            print_status "Удаляем остаточные конфиги cloud-init"
            apt-get purge -y cloud-init 2>/dev/null >/dev/null
            rm -rf /etc/cloud /var/lib/cloud 2>/dev/null
            print_ok "Зомби-cloud-init вычищен"
        fi
        ;;
    "")
        print_info "cloud-init не установлен и нет следов (норма для bare-metal)"
        ;;
    *)
        print_info "cloud-init состояние: $CLOUDINIT_STATE"
        ;;
esac

# --- Защита 5: GRUB_TIMEOUT минимум 2 секунды ---
# Если GRUB_TIMEOUT=0 — после неудачной загрузки нового ядра нет шанса
# выбрать старое из меню. Делаем минимум 2 секунды для шанса recovery.
print_status "Проверяем GRUB_TIMEOUT (для recovery опций при неудачной загрузке)..."
...
if [ -f /etc/default/grub ]; then
    ...
    if [ -n "$CURRENT_TIMEOUT" ] && [ "$CURRENT_TIMEOUT" -eq 0 ] 2>/dev/null; then
        ...
        print_status "Меняем на GRUB_TIMEOUT=2 (даёт 2 секунды на выбор в console)"
        sed -i 's/^GRUB_TIMEOUT=0/GRUB_TIMEOUT=2/' /etc/default/grub
        print_ok "GRUB_TIMEOUT обновлён до 2 секунд"
    elif [ -n "$CURRENT_TIMEOUT" ]; then
        print_ok "GRUB_TIMEOUT=$CURRENT_TIMEOUT (recovery возможен)"
    fi
fi

# --- Защита 6: VMware/Hyper-V предупреждение ---
# XanMod 7.x иногда конфликтует со старыми guest tools этих гипервизоров.
# Авто-фикс невозможен (нужно обновлять guest tools), только предупреждаем.
VIRT_TYPE=$(systemd-detect-virt 2>/dev/null)
case "$VIRT_TYPE" in
    "vmware")
        print_warn "Обнаружен VMware. XanMod ядро может конфликтовать со старыми VMware Tools."
        print_info "Если после ребута возникнут проблемы со временем или сетью — обновите open-vm-tools"
        ;;
    "microsoft")
...
# --- Защита 7: UFW информационная проверка ---
# Не трогаем правила (это намеренный выбор админа), но напоминаем проверить
# что отключение IPv6 и наши изменения не пересекаются с UFW.
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "Status: active"; then
    ...
    print_info "  • Что IPv6-правила не нужны (мы выключили IPv6)"
    print_info "  • Команда: ufw status verbose"
fi

# --- Защита 8: Апгрейд netplan accept-ra fix (для серверов с v4.6/v4.8) ---
# КОРНЕВАЯ ПРИЧИНА failed wait-online на KVM с virtio_net:
# networkd ждёт IPv6 RA даже при dhcp6: false. Если RA не приходит за 30с,
# wait-online падает по timeout. Старый override через --interface= не помогал —
# networkd всё равно держал интерфейс в состоянии "configuring".
#
# Правильный фикс: accept-ra: false + link-local: [ ipv4 ] в netplan.
# БЕЗ optional: true — на единственном интерфейсе он сам ломает wait-online
# (Launchpad #2060311, #2060689 на Noble/Debian13).
#
# Применяется к существующему 01-vpn-node-network.yaml если accept-ra нет.
# Только writeback в файл, БЕЗ netplan apply — применится при ребуте.
ACCEPTRA_FIXED=0
OUR_NETPLAN="$NETPLAN_DIR/01-vpn-node-network.yaml"
if [ -f "$OUR_NETPLAN" ]; then
    print_status "Проверяем netplan accept-ra fix..."
    if grep -qE '^[[:space:]]*accept-ra:[[:space:]]*false' "$OUR_NETPLAN" \
       && grep -qE '^[[:space:]]*link-local:[[:space:]]*\[' "$OUR_NETPLAN"; then
        print_ok "accept-ra fix уже на месте"
    else
        print_status "Применяю accept-ra fix к существующему netplan (от старой версии)"

        # Определяем интерфейс из существующего файла, fallback — DEFAULT_IFACE
        EXISTING_IFACE=$(awk '
            /ethernets:/ { in_eth=1; next }
            in_eth && /^[[:space:]]{4}[a-zA-Z0-9_-]+:[[:space:]]*$/ {
                line = $0
                sub(/^[[:space:]]+/, "", line)
                sub(/:[[:space:]]*$/, "", line)
                print line
        ...
        ' "$OUR_NETPLAN" 2>/dev/null)
        [ -z "$EXISTING_IFACE" ] && EXISTING_IFACE="$DEFAULT_IFACE"

        if [ -z "$EXISTING_IFACE" ]; then
            print_warn "Не определил интерфейс в $OUR_NETPLAN — пропускаем accept-ra fix"
        else
            cp -a "$OUR_NETPLAN" "$BACKUP_DIR/01-vpn-node-network.yaml.before-acceptra-fix"
            cat > "$OUR_NETPLAN" <<EOF
# Generated by vpn-node-setup v4.9 — accept-ra fix for systemd-networkd-wait-online
# Backup: $BACKUP_DIR/01-vpn-node-network.yaml.before-acceptra-fix
# Применится при следующем ребуте. НЕ запускай 'netplan apply' через SSH вручную.
network:
  version: 2
  renderer: networkd
  ethernets:
    $EXISTING_IFACE:
      ...
      link-local: [ ipv4 ]
EOF
            chmod 600 "$OUR_NETPLAN"

            # Валидация — netplan generate проверяет синтаксис без применения
            if netplan generate 2>&1 | grep -qiE "error|fail"; then
                print_error "netplan generate failed после accept-ra fix, откатываю"
                cp -a "$BACKUP_DIR/01-vpn-node-network.yaml.before-acceptra-fix" "$OUR_NETPLAN"
                netplan generate 2>/dev/null || true
            else
                print_ok "accept-ra fix готов, применится при следующем ребуте"
                ACCEPTRA_FIXED=1
            fi
        fi
    fi
...
# Чистка: удаляем wait-online override от v4.8 (если был — он не помогал реально)
OLD_OVERRIDE_FILE="/etc/systemd/system/systemd-networkd-wait-online.service.d/vpn-node-override.conf"
if [ -f "$OLD_OVERRIDE_FILE" ]; then
    print_status "Удаляю устаревший wait-online override от v4.8 (заменён на netplan accept-ra)"
    rm -f "$OLD_OVERRIDE_FILE"
    rmdir /etc/systemd/system/systemd-networkd-wait-online.service.d 2>/dev/null || true
    systemctl daemon-reload 2>/dev/null || true
    # Сбрасываем failed-state — после ребута wait-online должен сам ожить
    systemctl reset-failed systemd-networkd-wait-online.service 2>/dev/null || true
fi

# --- Защита 9: Нейтрализация 00-installer-config.yaml ---
# Subiquity-инсталлятор оставляет /etc/netplan/00-installer-config.yaml. Сейчас
# он не вредит, но это мина: в v4.0 ты ловил битые ссылки (enp1s0 на серверах
# с ens3) именно из таких автогенерированных файлов.
# Если у нас есть свой 01-vpn-node-network.yaml — заменяем installer-config
# на пустой каркас с комментарием.
#
# Безопасно потому что:
# - бэкап оригинала
# - наш 01-* всё равно имеет приоритет (alphabetic merge)
# - cloud-init управляет своим 50-cloud-init.yaml отдельно (его не трогаем)
INSTALLER_NEUTRALIZED=0
INSTALLER_YAML="$NETPLAN_DIR/00-installer-config.yaml"
if [ -f "$OUR_NETPLAN" ] && [ -f "$INSTALLER_YAML" ]; then
    if ! grep -q "neutralized by vpn-node-setup" "$INSTALLER_YAML" 2>/dev/null; then
        print_status "Нейтрализую $INSTALLER_YAML (предотвращение битых ссылок)"
        cp -a "$INSTALLER_YAML" "$BACKUP_DIR/00-installer-config.yaml.original"
        cat > "$INSTALLER_YAML" <<EOF
# neutralized by vpn-node-setup v4.9
# Original backed up to: $BACKUP_DIR/00-installer-config.yaml.original
# Active config: $OUR_NETPLAN
network:
  version: 2
  renderer: networkd
EOF
        chmod 600 "$INSTALLER_YAML"

        if netplan generate 2>&1 | grep -qiE "error|fail"; then
            print_error "netplan generate failed после neutralize, откатываю"
            cp -a "$BACKUP_DIR/00-installer-config.yaml.original" "$INSTALLER_YAML"
        else
            print_ok "00-installer-config.yaml нейтрализован"
            INSTALLER_NEUTRALIZED=1
        fi
    fi
fi

# --- Защита 10: ifupdown остатки ---
# Симптом: ifup@ens3.service failed после ребута. Причина: непустой
# /etc/network/interfaces или /etc/network/interfaces.d/* — старый ifupdown
# пытается управлять интерфейсом параллельно с активным systemd-networkd.
#
# Не удаляем пакет (может быть нужен для brctl на Docker-хостах), только
# переименовываем конфиги + reset-failed для уже упавших ifup@* юнитов.
IFUPDOWN_DISABLED=0
if systemctl is-active --quiet systemd-networkd 2>/dev/null; then
    INTERFACES_FILE="/etc/network/interfaces"
    INTERFACES_DIR="/etc/network/interfaces.d"

    # Проверяем /etc/network/interfaces на непустое содержимое (не только lo)
    if [ -f "$INTERFACES_FILE" ]; then
        NON_LO_LINES=$(grep -vE '^[[:space:]]*(#|$|auto[[:space:]]+lo\b|iface[[:space:]]+lo\b)' "$INTERFACES_FILE" 2>/dev/null | wc -l)
        if [ "$NON_LO_LINES" -gt 0 ]; then
            print_status "Найден непустой $INTERFACES_FILE при активном systemd-networkd — отключаю"
            cp -a "$INTERFACES_FILE" "$BACKUP_DIR/interfaces.disabled"
            mv "$INTERFACES_FILE" "${INTERFACES_FILE}.disabled-by-vpn-node-setup"
            IFUPDOWN_DISABLED=1
        fi
    fi

    # interfaces.d/* — переименовываем все непустые
    if [ -d "$INTERFACES_DIR" ]; then
        for f in "$INTERFACES_DIR"/*; do
            [ -f "$f" ] || continue
            case "$f" in
                *.disabled-by-vpn-node-setup) continue ;;
            esac
            [ -s "$f" ] || continue
            mkdir -p "$BACKUP_DIR/interfaces.d"
            cp -a "$f" "$BACKUP_DIR/interfaces.d/"
            mv "$f" "${f}.disabled-by-vpn-node-setup"
            IFUPDOWN_DISABLED=1
            print_status "Отключён: $f"
        done
    fi

    if [ "$IFUPDOWN_DISABLED" -eq 1 ]; then
        # Маскируем все failed ifup@* юниты — они уже не нужны
        for unit in $(systemctl list-units --all --failed --no-legend 2>/dev/null | awk '/ifup@/{print $2}'); do
            systemctl mask "$unit" 2>/dev/null || true
        done
        systemctl reset-failed 'ifup@*' 2>/dev/null || true
        print_ok "ifupdown остатки отключены (бэкап: $BACKUP_DIR/interfaces*)"
    fi
fi

# --- Финальное резюме шага ---
echo ""
print_info "Pre-flight проверки завершены:"
[ ${#NETPLAN_ISSUES[@]} -gt 0 ] && echo -e "    • Netplan: ${YELLOW}${#NETPLAN_ISSUES[@]} проблем найдено, $([ "$NETPLAN_FIXED" -eq 1 ] && echo "ИСПРАВЛЕНО" || echo "не исправлено")${NC}" || echo -e "    • Netplan: ${GREEN}OK${NC}"
[ "$NETWORKING_ACTIVE" -eq 1 ] && [ "$NETWORKD_ACTIVE" -eq 1 ] && echo -e "    • networking.service: ${YELLOW}конфликт устранён${NC}" || echo -e "    • networking.service: ${GREEN}OK${NC}"
[ -n "$DUPLICATES" ] && echo -e "    • fstab: ${YELLOW}дубликаты удалены${NC}" || echo -e "    • fstab: ${GREEN}OK${NC}"
[ "$ACCEPTRA_FIXED" -eq 1 ] && echo -e "    • netplan accept-ra: ${YELLOW}применён (ребут активирует)${NC}" || echo -e "    • netplan accept-ra: ${GREEN}OK${NC}"
[ "$INSTALLER_NEUTRALIZED" -eq 1 ] && echo -e "    • 00-installer-config.yaml: ${YELLOW}нейтрализован${NC}"
[ "$IFUPDOWN_DISABLED" -eq 1 ] && echo -e "    • ifupdown остатки: ${YELLOW}отключены${NC}"
echo ""
...
if [ "$CLOUD_DETECTED" != "none" ]; then
    ...
    # Различаем bare-metal от виртуалки без cloud-init
    VIRT_CHECK=$(systemd-detect-virt 2>/dev/null || echo "none")
    if [ "$VIRT_CHECK" = "none" ]; then
        print_ok "Bare-metal без cloud-init — безопасно удалять"
    else
        print_ok "Виртуализация ${VIRT_CHECK} без облачного cloud-init — типовая конфигурация VPS"
    fi
fi

# --- Бэкап существующих конфигов ---
print_status "Создаём бэкап существующих конфигов..."
# BACKUP_DIR уже создан в начале скрипта (для pre-flight checks)
...
print_header "ШАГ 5: ОТКЛЮЧЕНИЕ IPv6 (через GRUB cmdline)"

print_status "Отключаем IPv6 на уровне ядра (ipv6.disable=1)..."
print_info "Метод: GRUB cmdline — выгружает модуль ipv6 целиком"
print_info "Эффект: меньше RAM, меньше attack surface, AF_INET6 socket() вернёт ошибку"
echo ""

# === Метод 1: GRUB cmdline (основной — выгружает модуль) ===
IPV6_DISABLED_VIA_GRUB=0
if [ -f /etc/default/grub ] && (dpkg -l grub-pc &>/dev/null || dpkg -l grub-efi-amd64 &>/dev/null); then
    print_status "Модифицируем /etc/default/grub..."

    # Бэкап ДО изменений
    cp /etc/default/grub "$BACKUP_DIR/grub.backup" 2>/dev/null

    # Проверяем нет ли уже ipv6.disable
    if grep -qE 'ipv6\.disable=1' /etc/default/grub; then
        print_info "ipv6.disable=1 уже присутствует в GRUB cmdline"
        IPV6_DISABLED_VIA_GRUB=1
    else
        # Берём текущую строку GRUB_CMDLINE_LINUX_DEFAULT
        CURRENT_CMDLINE=$(grep '^GRUB_CMDLINE_LINUX_DEFAULT=' /etc/default/grub | head -1)

        if [ -n "$CURRENT_CMDLINE" ]; then
            # Извлекаем текущие параметры (между кавычками)
            CURRENT_PARAMS=$(echo "$CURRENT_CMDLINE" | sed -E 's/^GRUB_CMDLINE_LINUX_DEFAULT="([^"]*)"/\1/')
            # Удаляем устаревшие ipv6.disable_ipv6 если были
            CLEAN_PARAMS=$(echo "$CURRENT_PARAMS" | sed -E 's/\bipv6\.disable_ipv6=[01]\b//g' | tr -s ' ' | sed 's/^ //;s/ $//')
            # Добавляем ipv6.disable=1
            NEW_PARAMS="${CLEAN_PARAMS:+$CLEAN_PARAMS }ipv6.disable=1"
            # Заменяем строку
            sed -i -E "s|^GRUB_CMDLINE_LINUX_DEFAULT=\"[^\"]*\"|GRUB_CMDLINE_LINUX_DEFAULT=\"${NEW_PARAMS}\"|" /etc/default/grub
            print_ok "GRUB_CMDLINE_LINUX_DEFAULT обновлён: \"$NEW_PARAMS\""
        else
            # Строки нет вообще — добавляем
            echo 'GRUB_CMDLINE_LINUX_DEFAULT="ipv6.disable=1"' >> /etc/default/grub
            print_ok "GRUB_CMDLINE_LINUX_DEFAULT создан с ipv6.disable=1"
        fi

        # Применяем (update-grub перегенерит /boot/grub/grub.cfg)
        print_status "Перегенерируем grub.cfg..."
        if update-grub 2>&1 | tail -5; then
            print_ok "GRUB обновлён — ipv6 будет выгружен после ребута"
            IPV6_DISABLED_VIA_GRUB=1
        else
            print_error "update-grub провалился! Восстанавливаем бэкап..."
            cp "$BACKUP_DIR/grub.backup" /etc/default/grub 2>/dev/null
            print_info "GRUB восстановлен. IPv6 НЕ будет отключён через cmdline."
        fi
    fi
else
    print_info "GRUB не найден (systemd-boot? UEFI direct?) — используем только sysctl"
fi

# === Метод 2: sysctl (fallback / belt-and-suspenders) ===
# Применяется ДО ребута и страхует если GRUB-метод не сработал
print_status "Создаём sysctl fallback (на случай если ipv6 модуль всё же загрузится)..."
mkdir -p /etc/sysctl.d
cat > /etc/sysctl.d/99-disable-ipv6.conf <<EOF
# IPv6 disabled via GRUB cmdline (ipv6.disable=1)
# This is a fallback — applies only if module is somehow loaded
net.ipv6.conf.all.disable_ipv6 = 1
net.ipv6.conf.default.disable_ipv6 = 1
net.ipv6.conf.lo.disable_ipv6 = 1
EOF
print_ok "Sysctl fallback создан"

# === Force IPv4 priority в gai.conf (для приложений которые могут попробовать AAAA) ===
# Не критично если ipv6 модуль выгружен, но не помешает
...
if [ "$IPV6_DISABLED_VIA_GRUB" -eq 1 ]; then
    print_ok "IPv6 будет ПОЛНОСТЬЮ выгружен после перезагрузки (модуль не загрузится)"
else
    print_info "IPv6 будет отключён только через sysctl (модуль останется в памяти)"
fi
...
# Точное описание судьбы cloud-init на основе исходного состояния
if [ "$CLOUDINIT_ORIG_STATE" = "ii" ] && [ "$CLOUD_DETECTED" != "none" ]; then
    CLOUDINIT_FATE="сохранён ($CLOUD_DETECTED)"
elif [ "$CLOUDINIT_ORIG_STATE" = "ii" ]; then
    CLOUDINIT_FATE="удалён (bare-metal)"
elif [ "$CLOUDINIT_ORIG_STATE" = "un" ] || [ "$CLOUDINIT_ORIG_STATE" = "rc" ]; then
    CLOUDINIT_FATE="зомби-конфиги вычищены"
else
    CLOUDINIT_FATE="не установлен"
fi
echo -e "  ├─ ${GREEN}✔${NC} Удалены snap, apport, whoopsie (cloud-init: $CLOUDINIT_FATE)"
...
echo -e "  ├─ ${GREEN}✔${NC} Отключён IPv6 (GRUB cmdline + sysctl fallback)"
...
echo -e "  ${CYAN}lsmod | grep \"^ipv6 \"${NC}                       # Должно быть пусто (модуль выгружен)"
echo -e "  ${CYAN}cat /proc/cmdline | grep ipv6${NC}               # Должно показать ipv6.disable=1"
